...
export async function updateEvent(req, res) {
    ...
    try {
        const { rows: event } = await pool.query(
            'UPDATE  event SET event_name = $1, event_ort = $2, event_start = $3, event_ende = $4, event_beschreibung_kurz = $5, event_beschreibung_lang = $6,event_genre = $7, event_link = $8, event_bild = $9  WHERE user_id = $10 RETURNING *',
            [event_name,event_ort,event_start,event_ende,event_beschreibung_kurz,event_beschreibung_lang,event_genre,event_link,event_bild,id]
        );
        res.json(event);
    } catch (error) {
        console.log(error);
